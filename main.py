...
dbi = tools.HandleSql(config)
app = Client("my_bot", api_id=config['bot_config']['api_id'], api_hash=config['bot_config']['api_hash'],
             bot_token=config['bot_config']['bot_token'])  # create tg bot

    # 创建数据库及机器人连接
'''

    dbi.engine = create_dbi.engine(f'mysql+pymysql://%s:%s@%s:%s/%s' % (config['db_config']['db_user'],
                                                                config['db_config']['db_password'],
                                                                config['db_config']['db_host'],
                                                                config['db_config']['db_port'],
                                                                config['db_config']['db_name']))
    dbi.conn = pymysql.dbi.connect(host=config['db_config']['db_host'], user=config['db_config']['db_user'],
                           password=config['db_config']['db_password'], database=config['db_config']['db_name'],
                           port=config['db_config']['db_port'])
    dbi.cursor = dbi.conn.dbi.cursor()  # create database dbi.connect
    pd_invite_code = pd.read_sql_query('select * from invite_code;', dbi.engine)
    pd_config = pd.read_sql_query('select * from config;', dbi.engine)
    pd_user = pd.read_sql_query('select * from user;', dbi.engine)  # setup
'''
...
async def CreateCode(tgid=0):
    if IsAdmin(tgid=tgid):  # If you are a admin, you can create a code
        ...
        df_write.to_sql('invite_code', dbi.engine, index=False, if_exists='append')
        return code
    else:
...
async def invite(tgid=0, message=''):
    ...
    pd_invite_code = pd.read_sql_query('select * from invite_code;', dbi.engine)
    pd_user = pd.read_sql_query('select * from user;', dbi.engine)
    ...
    if used == 'T':
        ...
        dbi.cursor.execute(code_used)  # set the code has been used
        dbi.conn.commit()
        pd_invite_code = pd.read_sql_query('select * from invite_code;', dbi.engine)
        pd_user = pd.read_sql_query('select * from user;', dbi.engine)
        ...
        try:
            ...
            df_write.to_sql('user', dbi.engine, index=False, if_exists='append')  # add the user info
            pd_invite_code = pd.read_sql_query('select * from invite_code;', dbi.engine)
            pd_user = pd.read_sql_query('select * from user;', dbi.engine)
            return 'C'
        setcanrig = f"UPDATE `{db_name}`.`user` SET `canrig`='T' WHERE  `tgid`='{tgid}';"
        dbi.cursor.execute(setcanrig)  # update the status that can register
        dbi.conn.commit()
        pd_invite_code = pd.read_sql_query('select * from invite_code;', dbi.engine)
        pd_user = pd.read_sql_query('select * from user;', dbi.engine)
        return 'C'  # done
...
async def BanEmby(tgid=0, message='', replyid=0):
    if IsAdmin(tgid=tgid):
        if hadname(tgid=replyid) == 'B':
            ...
            dbi.cursor.execute(setbantime)  # update the status that cannot register
            dbi.conn.commit()
            ...
            if canrig(tgid=replyid):
                ...
                dbi.cursor.execute(setcanrig)  # update the status that cannot register
                dbi.conn.commit()
...
async def create(tgid=0, message=''):  # register with invite code
    ...
    try:
        ...
        df_write.to_sql('user', dbi.engine, index=False,
    ...
    dbi.cursor.execute(sqlcanrig)
    dbi.cursor.execute(sqlemby_name)
    dbi.cursor.execute(sqlemby_id)
    dbi.conn.commit()  # write it into database
    pd_invite_code = pd.read_sql_query('select * from invite_code;', dbi.engine)
    pd_user = pd.read_sql_query('select * from user;', dbi.engine)
    return r['Name'], NewPw
